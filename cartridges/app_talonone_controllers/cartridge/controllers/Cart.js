...
// Talonone  - Begin:
/**
 * For remove talon referral
 */
function removeReferral() {
    var cart = app.getModel('Cart').goc();
    Transaction.wrap(function () {
        cart.object.custom.referralCode = '';
        cart.calculate();
    });
    var url = URLUtils.url('Cart-Show').toString();
...
function submitForm() {
    ...
    formResult = cartForm.handleAction({
        ...
        //Add a referral if a referral was entered correctly and is active.
        'addReferral': function (formgroup) {
            var result = {
                cart: cart,
                EnableCheckout: true,
                dontRedirect: true
            ...
            if (TalonOnePreferences.isEnabled() && TalonOnePreferences.isReferralEnabled()) {
                ...
                if (formgroup.referralCode.htmlValue) {
                    //status = cart.addCoupon(formgroup.referralCode.htmlValue);

                    if (cart.object.custom.referralCode) {
                        result.ReferralStatus = {
                            code: Resource.msg('error.referral.used', 'talonOne', null),
                            error: true
                        };
                        return result;
                    }
                    ...
                    Transaction.wrap(function () {
                        cart.object.custom.referralCode = formgroup.referralCode.htmlValue;
                    });

                    var applyReferral = talonOneHelper.addTalononeReferral(cart.object, 'open');
                    if (applyReferral) {
                        result.ReferralStatus = {
                            code: applyReferral.message,
                        ...
                        // no status means valid but inactive referral
                        result.ReferralError = 'NO_ACTIVE_PROMOTION';
                    }
                } else {
                    // no coupon code supplied
                    result.ReferralError = 'REFERRAL_CODE_MISSING';
                }
            } else {
                result.ReferralStatus = {
                    code: Resource.msg('error.referral.disabled', 'talonOne', null),
                    error: true
                };
            }
            return result;
        ...
        'deleteCoupon': function (formgroup) {
            Transaction.wrap(function () {
                ...
                // Talonone - Begin: Removing/Updating free lineitem based on the talonone response effects. Also updatng the basket custom attribute with coupon codes
                if (TalonOnePreferences.isEnabled()) {
                    var couponLineItem = formgroup.getTriggeredAction().object;
                    talonOneHelper.removeFreeLineItemFromBasket(cart.object, couponLineItem);

                    var couponCodes = [];
                    if (!empty(cart.object.custom.talononeCouponCodes)) {
                        var appliedCoupons = cart.object.custom.talononeCouponCodes;
                        ...
                        cart.object.custom.talononeCouponCodes = couponCodes;
                    }
                ...
                // Talonone - End: Removing/Updating free lineitem based on the talonone response effects. Also updatng the basket custom attribute with coupon codes
        ...
        'deleteProduct': function (formgroup) {
            ...
            if (TalonOnePreferences.isEnabled()) {
                talonOneHelper.talononeRejectedFreeItem(cart.object, formgroup.getTriggeredAction().object);
            }
                ...
                for (var i = 0; i < session.forms.cart.shipments.childCount; i++) {
                    ...
                    for (var j = 0; j < shipmentItem.items.childCount; j++) {
                        ...
                        if (item.quantity.value === 0) {
                            // Talonone - Begin: On Talonone free item product lineitem removal, we are updating the rejected free item id's to Talonone request.
                            if (TalonOnePreferences.isEnabled()) {
                                talonOneHelper.talononeRejectedFreeItem(cart.object, item.object);
                            }
                            // Talonone - End: On Talonone free item product lineitem removal, we are updating the rejected free item id's to Talonone request.
    ...
    if (formResult) {
        ...
        // Talonone  - Begin:
        var talon = talonOneHelper.talonConfig(false, 'Cart', formResult.cart.object.custom.referralCode);
        // Talonone  - End:
        ...
        if (formResult.dontRedirect) {
            app.getView({
                ...
                ReferralStatus: formResult.ReferralStatus,
                ReferralError: formResult.ReferralError,
                talon: talon
            }).render('checkout/cart/cart');
        } else {
...
/** Remove Referral
 * @see {@link module:controllers/Cart~removeReferral} */
exports.removeReferral = guard.ensure(['get'], removeReferral);
