...
var CartModel = AbstractModel.extend({
     ...
     * @param {Boolean} isFreeDiscountItem - The value set to identify whether the product are added from Talonone as a free product item.
     */
    addProductItem: function (product, quantity, productOptionModel, isFreeDiscountItem) {
        var cart = this;
        Transaction.wrap(function () {
            var i;
            if (product) {
                ...
                // Talonone - Begin: Avoiding unnecessary cart calculate calls, to avoid quota limitation while free items are added to cart based on talonone response effect
                if (!(TalonOnePreferences.isEnabled() && isFreeDiscountItem)) {
                    cart.calculate();
                }
                // Talonone - End: Avoiding unnecessary cart calculate calls, to avoid quota limitation while free items are added to cart based on talonone response effect
            }
        });
    },
        ...
        if (couponCode) {
            ...
            try {
                if (TalonOnePreferences.isEnabled()) {
                    ...
                    addCouponToBasketResult = talonOneHelper.addTalononeCoupon(cart.object, couponCode, 'open');

                    if (!(addCouponToBasketResult instanceof dw.order.CouponLineItem)) {
                        return {CouponStatus: 'COUPON_CODE_UNKNOWN'};
                    }
                } else {
                ...
                return {CouponStatus: 'COUPON_CODE_ALREADY_IN_BASKET'};
            }
