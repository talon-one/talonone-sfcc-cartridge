    ...
    prepareView: function () {
        ...
        if (cart) {
           ...
           // Refreshes the cart calculation.
            ...
            // Talonone - Begin: Recalculating the cart items, when a discount free lineitem are added with respect to talonone response effects
            var TalonOnePreferences = require('*/cartridge/scripts/util/talonOnePreferences');
            if (TalonOnePreferences.isEnabled()) {
                var cartHelper = require('*/cartridge/scripts/cart/cartHelpers');
                cartHelper.addFreeItemCartCalculate(cart);

                if (session.privacy.isTalonOneAddFreeItemError) {
                    this.isTalonOneAddFreeItemerror = true;
                }
                ...
                // Update the talonone referrals
                var talonOneHelper = require('*/cartridge/scripts/helper/talonOneHelper');
                var talon = talonOneHelper.talonConfig(false, 'Cart', cart.custom.referralCode);
                this.talon = talon;
            }
            // Talonone - End: Recalculating the cart items, when a discount free lineitem are added with respect to talonone response effects
